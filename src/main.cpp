...
static HWND& gameWnd = *(HWND*)0x7897A4;
...
static BOOL InitMouseFix()
{
	// Attempt to create a(nother - other DLLs could have done this themselves too) WndProc subclass
	oldWndProc = (WNDPROC)SetWindowLong(gameWnd, GWL_WNDPROC, (LONG)MouseFixWndProc);
	...
	// Mouse lock fix
	//
	// Taken from Silent's SilentPatch:
	...
	GameMem<DWORD>(0x601740).value = 0x00C3C030;
